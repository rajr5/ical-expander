...
const icsWithTimezone = fs.readFileSync(path.join(__dirname, 'with_timezone.ics'), 'utf-8');
...
it('should auto-register timezones', async function() {
  const icalExpander = new IcalExpander({ ics: icsWithTimezone });

  assert.equal(icalExpander.timezones.length, 6);
});

it('should not auto-register if manually registered', async function() {
  await icsExpander.registerTimezones();
  const icalExpander = new IcalExpander({ ics: icsWithTimezone });

  assert.notEqual(icalExpander.timezones.length, 6);
});

it('should show first date', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: icaljsIssue257 }).between(
    new Date('2016-07-24T00:00:00.000Z'),
    new Date('2016-07-26T00:00:00.000Z'),
  );
...
it('should show recurring modified date', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: icaljsIssue257 }).between(
...
it('should show nothing at recurring exdate', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: icaljsIssue257 }).between(
    new Date('2016-08-07T00:00:00.000Z'),
    new Date('2016-08-10T00:00:00.000Z'),
  );
...
it('should parse issue 285 case correctly', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: icaljsIssue285 }).between(
    new Date('2017-01-03T00:00:00.000Z'),
    new Date('2017-01-25T00:00:00.000Z'),
  );
...
it('should parse all recurring events without going on forever', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: recurIcs }).all();
...
it('should correctly parse an ical file with invalid start/end dates', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: invalidDates, skipInvalidDates: true }).all();
...
it('should include events that are partially between two dates', async function() {
  await icsExpander.registerTimezones();
  const eventIdsBetween = ['3', '4', '5', '6', '7'];
  const occurrenceIdsBetween = ['8'];

  const events = new IcalExpander({ ics: betweenTestCalendar }).between(
    new Date('2018-05-02T00:00:00.000Z'),
    new Date('2018-05-02T23:59:59.999Z'),
  );
  assert.equal(events.events.length, 5);
  assert.equal(events.occurrences.length, 1);

  events.events.forEach(event => {
    assert.ok(eventIdsBetween.findIndex(id => id === event.uid) >= 0, `${event.uid} is not a valid event between provided dates`);
  });
...
it('should support open ended limits', async function() {
  await icsExpander.registerTimezones();
  const events = new IcalExpander({ ics: allDayEventCalendar, maxIterations: 10 }).between(new Date('2018-05-19T23:59:00.000Z'));
